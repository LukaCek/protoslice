...
def run_orcaslicer(input_file_path, process, filament):
    ...
    while True:
        ...
        app.logger.info(f"Resolved process_config: {process_config}")
        app.logger.info(f"Resolved filament_config: {filament_config}")

        # Defensive: ensure process is from process dir, filament from filament dir
        if not os.path.exists(process_config):
            app.logger.error(f"Process config does not exist: {process_config}")
            return False
        if not os.path.exists(filament_config):
            app.logger.error(f"Filament config does not exist: {filament_config}")
            return False
        ...
        app.logger.info("Running command: %s", ' '.join(cmd))
...
def home():
    if request.method == 'POST':
        ...
        app.logger.info(f"filament: '{filament}' process: '{process}'")

        # Validate file presence
        if not stlFile:
            app.logger.error("'stlFile' is required")
            return "'stlFile' is required", 400
        
        # Validate file extension
        if not stlFile.filename.lower().endswith('.stl'):
            app.logger.error("Uploaded file is not an STL file")
        ...
        app.logger.info(f"Running OrcaSlicer with file: {input_file_path}, filament: {filament}, process: {process}")
        ...
        if not success:
            ...
            with open('result.json', 'rb') as f:
                ...
                if error_string == "Success.":
                    app.logger.info("Slicing completed successfully.")
    ...
    return render_template('index.html', filements=filament, processes=process)

@app.route('/3mf', methods=['GET'])
# TODO: refactor so it returns Metadata dir only
def get_3mf():
...
if __name__ == '__main__':
    app.run(debug=True, port=5252, host='0.0.0.0')